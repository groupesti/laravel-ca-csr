...
final class CsrManager implements CsrManagerInterface
{
    ...
    public function create(
        ...
        CaLog::log('csr_creation', 'info', "CSR created: {$subjectCn}", [
            'csr_id' => $csr->id,
            'key_id' => $key->id,
            'ca_id' => $key->ca_id,
            'template_id' => $template?->id,
    ...
    public function import(string $csrPem): Csr
    {
        ...
        CaLog::log('csr_import', 'info', "CSR imported: {$subjectDn['CN'] ?? 'unknown'}", [
            'csr_id' => $csr->id,
            'subject_dn' => $subjectDn,
            'san_count' => count($sanList),
        ]);
    ...
    public function approve(Csr $csr, ?string $approvedBy = null): Csr
    {
        ...
        CaLog::log('csr_approval', 'info', "CSR approved: {$csr->subject_dn['CN'] ?? 'unknown'}", [
            'csr_id' => $csr->id,
            'approved_by' => $approvedBy ?? $this->resolveCurrentUser(),
        ]);
    ...
    public function reject(Csr $csr, string $reason, ?string $rejectedBy = null): Csr
    {
        ...
        CaLog::log('csr_rejection', 'info', "CSR rejected: {$csr->subject_dn['CN'] ?? 'unknown'}", [
            'csr_id' => $csr->id,
            'reason' => $reason,
            'rejected_by' => $rejectedBy ?? $this->resolveCurrentUser(),
        ]);
