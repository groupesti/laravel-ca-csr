...
final class CsrValidator implements CsrValidatorInterface
{
    ...
    public function validate(Csr $csr): bool
    {
        ...
        if ($isValid) {
            CaLog::log('csr_validation', 'info', "CSR validation passed: {$csr->subject_dn['CN'] ?? 'unknown'}", [
                'csr_id' => $csr->id,
            ]);
        } else {
            CaLog::log('csr_validation', 'warning', "CSR validation failed: {$csr->subject_dn['CN'] ?? 'unknown'}", [
                'csr_id' => $csr->id,
                'errors' => $this->errors,
            ]);
        }
